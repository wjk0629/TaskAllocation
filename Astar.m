...
for i=1:msize(1)
    for j=1:msize(2)
        h(i,j)=sqrt((i-goal.x)^2+(j-goal.y)^2);
        if map(i,j)==1
            h(i,j)=h(i,j)+10000;
        end
    end
...
open=[initial.x, initial.y, h(initial.x,initial.y), -1, -1];
%open nodes,[X(x coordinate of the cell/column),Y( y coordinate of the cell/row),f(sum of weight of all previous nodes and included this+heuristic), x
...
while ~isequal(current(1:2),[goal.x goal.y])
            ...
            if BinA(open(:,1:2),children(i,1:2))~=0 %if the child is already present in the open but with a more expensive cost
                ...
                if BinA(closed(:,1:2),children(i,1:2))==0
                    ...
                    if (children(i,3)<closed(BinA(closed(:,1:2),children(i,1:2)),3))
                        if BinA(open(:,1:2),children(i,1:2))
...
node=current;
path=current(1:2);
while ~isequal(node(4:5),[initial.x initial.y])
    path=[node(4:5); path];
